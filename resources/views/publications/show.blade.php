            ...
            <div class="col-md-6">
                ...
                <div class="d-flex justify-content-between mt-2">
                    <a href="{{ route('texts.create', ['publication_id' => $publication->id]) }}"
                        class="btn btn-info text-light flex-grow-1 m-1"><i class='bx bx-envelope'></i>
                        {{ __('publications_show.contact') }}</a>
                    @php
                    ...
                    <form
                        ...
                        @if ($isFavorited)
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger w-100"><i class='bx bxs-heart'></i>
                                {{ __('publications_show.unfavorite') }}</button>
                        @else
                            <button type="submit" class="btn btn-danger w-100"><i class='bx bx-heart'></i>
                                {{ __('publications_show.favorite') }}</button>
                        @endif
                    </form>
                    <button type="button" class="btn btn-warning text-light flex-grow-1 m-1" data-bs-toggle="modal"
                        data-id="{{ $publication->id }}" data-bs-target="#reportModal"><i class='bx bx-flag'></i>
                        {{ __('publications_show.report') }}</button>
                    <a href="{{ route('index') }}" class="btn btn-secondary flex-grow-1 m-1"><i
                            class='bx bx-home-alt-2'></i> {{ __('publications_show.home') }}</a>
                </div>
            </div>
                ...
                <div class="flex-grow-1 mb-2">
                    <div class="card">
                        <div class="card-body border rounded p-3">
                            ...
                            <div class="detail-item"><strong>{{ __('publications_show.author') }}:</strong>
                                {{ $publication->user->username }}</div>
                            <div class="detail-item"><strong>{{ __('publications_show.type') }}:</strong>
                                {{ __('publications_show.type_values.' . $publication->type) }}</div>
                            <div class="detail-item"><strong>{{ __('publications_show.animal_type') }}:</strong>
                                {{ __('publications_show.animal_type_values.' . $publication->type_animal) }}</div>
                            <div class="detail-item"><strong>{{ __('publications_show.size') }}:</strong>
                                {{ __('publications_show.publication_size.' . strtolower($publication->size)) }}</div>
                            <div class="detail-item"><strong>{{ __('publications_show.date') }}:</strong>
                            ...
                            <div class="detail-item"><strong>{{ __('publications_show.description') }}:</strong>
                                ...
                                @if (strlen($publication->description) > 40)
                                    <a href="#" data-bs-toggle="modal"
                                        data-bs-target="#descriptionModal">{{ __('publications_show.read_more') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                        data-bs-dismiss="modal">{{ __('publications_show.close') }}</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <form id="reportForm" method="POST" action="{{ route('publications.report', $publication->id) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="reason" name="reason" required>
                                <option value="Contenido inapropiado">
                                    {{ __('publications_show.report_reason_inappropriate') }}</option>
                                <option value="Información incorrecta">
                                    {{ __('publications_show.report_reason_incorrect') }}</option>
                        ...
                        <div class="mb-3">
                            <label for="additional_info"
                                class="form-label">{{ __('publications_show.additional_info') }}</label>
                            <textarea class="form-control" id="additional_info" name="additional_info" rows="3" maxlength="200"></textarea>
                        </div>
                        <button type="submit" class="btn btn-danger">{{ __('publications_show.send_report') }}</button>
                    </form>
                </div>
            </div>
        </div>
